...
export function CompletionDialog({
  ...
  return (
    <div role="status" className="mt-6 rounded-lg border border-term-green/40 bg-term-green/5 p-6 text-center">
      ...
      <div className="text-lg font-semibold text-term-text">Puzzle solved!</div>
      <p className="mt-1 text-sm text-term-muted">
        {formatTime(elapsedSeconds)} · {moveCount} {moveCount === 1 ? "move" : "moves"}
      </p>
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        ...
        >
          <ImageIcon className="h-4 w-4" /> Choose another image
        </button>
      </div>
    </div>
  );

...
/**
 * The finance app's shell — bottom nav, quick-add sheet, toast, PWA update
 * banner, debug panel — shared by both FinanceApp (Demo Mode,
 * /expenses/demo) and RealFinanceApp (Real Mode, /expenses/app). Extracted
 * from what was originally FinanceApp.tsx's entire body so the two modes
...
export default function FinanceAppShell({ repository, basePath }: { repository: FinanceRepository; basePath: string }) {
  ...
  // Created once per mount, not at module scope — avoids holding a query
  // cache open for the lifetime of the whole site when the user has never
  // visited the finance app.
